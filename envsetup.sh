...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^glade_") ; then
       GLADE_BUILD=$(echo -n $1 | sed -e 's/^glade_//g')
       export BUILD_NUMBER=$((date +%s%N ; echo $GLADE_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10)
    else
       GLADE_BUILD=
    fi
    export GLADE_BUILD
...
function print_lunch_menu()
{
    ...
    echo
    if [ "z${GLADE_DEVICES_ONLY}" != "z" ]; then
...
function breakfast()
{
    ...
    for f in `/bin/ls vendor/glade/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
...
function eat()
{
    ...
    if (adb shell cat /system/build.prop | grep -q "ro.glade.device=$GLADE_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell cat /system/build.prop | grep -q "ro.glade.device=$GLADE_BUILD") || [ "$FORCE_PUSH" == "true" ];
    then
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $GLADE_FIXUP_COMMON_OUT ]; then
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/glade/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
